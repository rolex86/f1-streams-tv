...
public class MainActivity extends Activity {
    ...
    private static final String USER_AGENT = "Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Mobile Safari/537.36";
    ...
    private void buildUi() {
        ...
        web.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                if (pendingPlayback != null && url.startsWith("https://f1live.dpdns.org/")) {
                    int token = playbackToken;
                    handler.postDelayed(() -> {
                        if (token == playbackToken && pendingPlayback != null) selectForPlayback(pendingPlayback);
                    }, 900);
                } else if (collecting && url.startsWith("https://f1live.dpdns.org/")) {
                    int token = collectionToken;
                    handler.postDelayed(() -> collectProvider(token, 0), 1400);
                }
            }
        ...
        root.addView(web, new LinearLayout.LayoutParams(dp(2), dp(2)));
        setContentView(root);
    }
    ...
    private void refresh() {
        ...
        status.setText("Načítám streamy přímo z webu…");
        web.stopLoading();
        web.loadUrl(PAGE);
    }
    ...
    private void collectProvider(int token, int retry) {
        ...
        clickText(group, clicked -> handler.postDelayed(() -> scrapeNames(group, token, retry), clicked ? 700 : 250));
    }

    private void scrapeNames(String group, int token, int retry) {
        if (!collecting || token != collectionToken) return;
        String js = "(function(){" +
                "function v(e){var r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!='none'&&s.visibility!='hidden';}" +
                "var a=[],all=[].slice.call(document.querySelectorAll('body *'));" +
                "all.forEach(function(e){if(!v(e))return;var t=(e.innerText||'').trim();if(!t||t.length>220||!/(^|\\n)online($|\\n)/i.test(t))return;" +
                "var l=t.split(/\\n+/).map(function(x){return x.trim();}).filter(Boolean).filter(function(x){return !/^online$/i.test(x)&&!/may contain ads/i.test(x);});" +
                "if(l.length&&l[0].length<100)a.push({n:l[0],z:t.length});});" +
        ...
        web.evaluateJavascript(js, value -> {
            ...
            if (names.isEmpty() && retry < 4) {
                handler.postDelayed(() -> collectProvider(token, retry + 1), 900);
                return;
            }
            ...
            if (providerIndex < GROUPS.length) {
                handler.postDelayed(() -> collectProvider(token, 0), 350);
            } else {
    ...
    private void finishCollection(int token) {
        ...
        adapter.notifyDataSetChanged();
        status.setText(collected.isEmpty() ? "Web nenačetl žádné streamy." : "Dostupné streamy: " + collected.size());
        list.requestFocus();
    }

    private void clickText(String text, BoolCallback callback) {
        String q = JSONObject.quote(text);
        String js = "(function(){function v(e){var r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!='none'&&s.visibility!='hidden';}" +
                "var q=" + q + ",a=[].slice.call(document.querySelectorAll('body *')).filter(function(e){return v(e)&&(e.innerText||'').trim()===q;});" +
                "a.sort(function(x,y){return (x.innerText||'').length-(y.innerText||'').length;});if(!a.length)return false;a[0].click();return true;})()";
    ...
    private void clickChannel(String name, BoolCallback callback) {
        ...
        String js = "(function(){function v(e){var r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!='none'&&s.visibility!='hidden';}" +
                "var q=" + q + ",a=[].slice.call(document.querySelectorAll('body *')).filter(function(e){if(!v(e))return false;var t=(e.innerText||'').trim();if(!t||t.length>220)return false;var l=t.split(/\\n+/).map(function(x){return x.trim();}).filter(Boolean);return l.length&&l[0]===q;});" +
                "a.sort(function(x,y){return (x.innerText||'').length-(y.innerText||'').length;});if(!a.length)return false;a[0].click();return true;})()";
        web.evaluateJavascript(js, value -> callback.done("true".equalsIgnoreCase(value)));
    }
